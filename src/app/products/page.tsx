...
const PRODUCT_CATEGORIES = [
  ...
  {
    title: 'Radar Modules',
    tag: 'Detection & Safety',
    tagColor: 'bg-amber-500',
    href: '/radar-modules',
    image: '/radar/bd4101a-front.png',
    intro: 'Radar modules for microwave presence sensing and blind-spot safety applications.',
    summary: 'This category contains radar products for smart devices, vehicle safety, and future radar platform expansion.',
    bullets: ['Microwave radar', 'BSD safety modules', 'More radar variants coming'],
  },
]
  ...
  return (
    <>
                    ...
                    <div className="p-3 border-r border-gray-100 bg-gradient-to-b from-green-50 to-white">
                      <a href="/products" className="flex items-start gap-3 px-3 py-3 rounded-xl bg-white border border-green-100 shadow-sm transition-colors">
                        ...
                        <div>
                          ...
                          <p className="text-gray-500 text-xs leading-relaxed mt-1">Browse all product families from one category overview page.</p>
                        </div>
                      </a>
      ...
      <main className="pt-[68px]">
          ...
          <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div className="max-w-3xl">
              ...
              <h1 className="text-4xl lg:text-5xl font-bold text-gray-900 leading-tight mb-5">
                Products<br />
                <span className="text-green-600">Category Overview</span>
              </h1>
              <p className="text-gray-500 text-lg leading-relaxed max-w-2xl">
                This page only shows product categories. Right now there are two major families, and more categories can be added here later without changing the overall structure.
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {PRODUCT_CATEGORIES.map((category) => (
                <a key={category.title} href={category.href} className="rounded-3xl border border-gray-100 bg-gray-50 overflow-hidden card-hover">
                  <div className="grid grid-cols-1 md:grid-cols-[240px_1fr]">
                    <div className="bg-white border-b md:border-b-0 md:border-r border-gray-100 flex items-center justify-center p-6">
                      <img src={category.image} alt={category.title} className="w-full max-w-[240px] h-auto object-contain" />
                    </div>
                    <div className="p-7 lg:p-8">
                      <span className={`inline-flex ${category.tagColor} text-white text-xs font-bold px-2.5 py-1 rounded-full mb-4`}>{category.tag}</span>
                      <h2 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-3">{category.title}</h2>
                      <p className="text-green-700 font-medium text-sm mb-3">{category.intro}</p>
                      <p className="text-gray-500 text-sm leading-relaxed mb-5">{category.summary}</p>
                      <div className="flex flex-wrap gap-2 mb-6">
                        {category.bullets.map((item) => (
                          <span key={item} className="bg-white text-gray-600 text-xs font-medium px-3 py-1.5 rounded-lg border border-gray-200">{item}</span>
                        ))}
                      </div>
                      <div className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold px-5 py-3 rounded-xl transition-colors">
                        View Category Page
        ...
        <section className="pb-24 bg-white">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="rounded-3xl border border-dashed border-green-200 bg-green-50 p-8 lg:p-10">
              <div className="max-w-3xl">
                <p className="text-green-700 text-sm font-semibold uppercase tracking-wider mb-3">Expandable Structure</p>
                <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">Built for More Categories Later</h2>
                <p className="text-gray-600 leading-relaxed mb-6">
                  The `Products` page now acts as a stable category hub. If you add new families in the future, such as new module lines or special application products, they can be added here as another category card without changing the page logic.
                </p>
                <div className="flex flex-wrap gap-3">
                  {['Sensor Modules', 'Radar Modules', 'Future Category 03', 'Future Category 04'].map((item) => (
                    <span key={item} className="bg-white text-gray-700 text-sm font-medium px-4 py-2 rounded-xl border border-green-100">{item}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        <section className="py-24 relative overflow-hidden">
          ...
          <div className="relative max-w-4xl mx-auto px-6 lg:px-8 text-center">
            ...
            <p className="text-gray-400 text-lg mb-10 max-w-2xl mx-auto">
              Tell us your application, and we can direct you to the right module family or recommend the best starting point.
            </p>
